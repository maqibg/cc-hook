...
pub async fn generate(cfg: &Config, client: &reqwest::Client, content: &str) -> String {
    ...
    if cfg.debug { eprintln!("[cc-hook] AI 请求: {url}"); }
    ...
    match result {
        Ok(Ok(data)) => {
            ...
            if cfg.debug { eprintln!("[cc-hook] AI 响应无内容: {data}"); }
        }
        Ok(Err(e)) => { if cfg.debug { eprintln!("[cc-hook] AI 请求失败: {e}"); } }
        Err(_) => { if cfg.debug { eprintln!("[cc-hook] AI 请求超时"); } }
    }
