...
pub async fn generate(cfg: &AiSummaryConfig, content: &str, debug: bool) -> String {
    ...
    let client = match build_client(&cfg.proxy_url, cfg.timeout_ms) {
        ...
        Err(error) => {
            if debug {
                eprintln!("[cc-hook] AI client 失败: {}", error.code);
            }
            return fallback;
        }
    };
    ...
    match result {
        Ok(Ok(data)) => {
            ...
            if debug {
                eprintln!("[cc-hook] AI 响应无内容");
            }
        }
        Ok(Err(code)) => {
            if debug {
                eprintln!("[cc-hook] AI 请求失败: {code}");
            }
        }
        Err(_) => {
            if debug {
                eprintln!("[cc-hook] AI 请求失败: request-timeout");
            }
        }
    }
    fallback
