...
fn parse_channels() -> Vec<Channel> {
    ...
    for id in &prefixes {
        ...
        let mut ok = true;
        if ch_type == "telegram" {
            if ch.token.is_none() { eprintln!("[cc-hook] {id}_TOKEN 未配置"); ok = false; }
            if ch.chat_id.is_none() { eprintln!("[cc-hook] {id}_CHAT_ID 未配置"); ok = false; }
        } else if ch.webhook_url.is_none() {
            eprintln!("[cc-hook] {id}_WEBHOOK_URL 未配置"); ok = false;
        }
...
impl Config {
    pub fn load() -> Self {
        ...
        let cfg = Config {
            ...
            min_duration: env_or("MIN_DURATION", "0").parse().unwrap_or(0),
        ...
        if cfg.debug {
            eprintln!("[cc-hook] proxy={}, channels={}", if cfg.proxy.is_empty() { "(空)" } else { &cfg.proxy }, cfg.channels.len());
        }
        cfg
    }
}
